import { X, MapPin, Clock, ChevronRight, User, Bot, Ticket } from "lucide-react";
...
export function IssueBottomSheet({ issue, role, onClose, onViewDetails, onAssign }: Props) {
  return (
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 pr-4">
            <div className="flex items-center gap-2 mb-1">
              <span style={{ fontSize: '18px' }}>{CATEGORY_ICONS[issue.category]}</span>
              <span className="text-xs text-gray-500" style={{ fontFamily: 'Inter, sans-serif', fontWeight: 500 }}>
                {issue.id}
              </span>
              {issue.source === 'ai' && (
                <span className="flex items-center gap-0.5 text-xs px-2 py-0.5 rounded-full bg-purple-100 text-purple-700" style={{ fontFamily: 'Inter, sans-serif', fontWeight: 600 }}>
                  <Bot size={11} /> AI
                </span>
              )}
            </div>
        ...
        {role === 'user' ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-3">
              ...
              {issue.source === 'ai' && (
                <motion.button whileTap={TAP} className="flex-1 py-3 rounded-2xl text-white text-sm" style={{ background: '#7C3AED', fontFamily: 'Inter, sans-serif', fontWeight: 600 }}>
                  Review AI
                </motion.button>
              )}
            </div>
